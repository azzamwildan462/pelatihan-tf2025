...
#include "ros2_utils/help_udp.hpp"
...
public:
    ...
    //----Subscriber
    rclcpp::Subscription<ros2_interface::msg::Stm32FromPc>::SharedPtr sub_stm32_from_pc;
    //----Publisher
    rclcpp::Publisher<ros2_interface::msg::Stm32ToPc>::SharedPtr pub_stm32_to_pc;
    //----Help
    HelpLogger logger;
    HelpUDP udp;
    ...
    uint16_t encoder_odometry0; // kiri
    ...
    IOSTM32() : Node("io_stm32")
    {
        ...
        //----Subscriber
        sub_stm32_from_pc = this->create_subscription<ros2_interface::msg::Stm32FromPc>(
            "stm32/from_pc", 10, std::bind(&IOSTM32::cllbck_sub_stm32_from_pc, this, std::placeholders::_1));
        //----Publisher
        pub_stm32_to_pc = this->create_publisher<ros2_interface::msg::Stm32ToPc>("stm32/to_pc", 10);
        //----Help
        if (!logger.init())
        {
            rclcpp::shutdown();
        }
        if (!udp.init_as_client(ip, port))
        {
            rclcpp::shutdown();
        }
    }
    ...
    cllbck_tim_50hz()
    {
        ...
        int len_data_to_pc = udp.recv(rx_buffer, 31);
        ...
        memcpy(&encoder_odometry0, rx_buffer + 27, 2);
        memcpy(&encoder_odometry1, rx_buffer + 29, 2);

        // logger.info("halo: %d", encoder_roda0);
        ...
        static ros2_interface::msg::Stm32ToPc msg_stm32_to_pc;
    ...
    void
    cllbck_sub_stm32_from_pc(const ros2_interface::msg::Stm32FromPc::SharedPtr msg)
    {
        mode = msg->mode;
        dx = msg->dx;
        dy = msg->dy;
